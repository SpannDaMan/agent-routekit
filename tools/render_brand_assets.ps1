...
$manifestFiles = @(Get-ChildItem -LiteralPath $pluginRoot -Recurse -File -Filter 'Logo Generation Manifest 220826.json')
if ($manifestFiles.Count -ne 1) {
    throw "Expected exactly one Logo Generation Manifest 220826.json below $pluginRoot; found $($manifestFiles.Count)."
}
...
try {
    ...
    # Composer icon: a transparent, large-safe-fill resize of the accepted master.
